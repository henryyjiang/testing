...
class PSO():
    ...
    def obj_func(self):
        ...
        return new_atoms, [i[0] for i in energy_loss]

    def f(self, x):
        new_atoms, j = self.obj_func()
        print(new_atoms)
        print(j)
        for i in range(len(j)):
            if j[i] < self.best_loss:
                self.best_pos = new_atoms[i]
                self.best_loss = j[i]
    ...
    def run(self):
        for i in range(5):
            ...
            for i in range(particles):
                ...
                init_pos = [float(i) for l in init_atoms.positions for i in l]
                init_positions[i] = np.array(init_pos)
            filename = f"init_structure_psolocal_{i+1}" + ".cif"
            ase.io.write(filename, self.dimensions_to_atoms(init_positions[0]))
            ...
            cost, pos = self.optimizer.optimize(self.f, iters=100)
            ...
            plt.savefig(f'best_losses_psolocal_{i+1}.png')
            plt.close()
            ...
            plt.savefig(f'avg_losses__psolocal_{i+1}.png')
            plt.close()
