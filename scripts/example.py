...
predictor_batch = BasinHoppingBatch(forcefield, hops=20, steps=100, dr=0.6, optimizer='Adam', batch_size=30)
...
for i in range(0, max_iterations):
    ...
    for i, energy_list in enumerate(energies):
        points = [(j, energy) for j, energy in enumerate(energy_list) if -4 <= energy <= 0]
        x, y = zip(*points)
        accepts_filtered = [accepts[i][j] for j, _ in points]
        plt.scatter(x, y, label=f'Structure {i + 1}',
                    color=['g' if a else 'r' for a in accepts_filtered])
